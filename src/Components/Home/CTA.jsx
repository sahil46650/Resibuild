    ...
    useEffect(() => {
        ...
        fetch("https://resi.build/backend/wp-json/wp/v2/pages/41")
            ...
            .then((data) => {
                ...
                if (data?.acf) {
                    setCtaHeading(data?.acf?.cta?.title || "Default Heading");
                    setCtaButtonText(data?.acf?.cta?.button_text || "Default Button Text");
                    setCtaButtonLink(data?.acf?.cta?.button_url || "#");
                } else {
        ...
        <div className="ready-to-talk-area ptb-130">
            <div className="container">
                <div className="ready-to-talk-content wow animate__animated animate__fadeInUp delay-0-2s">
                    <h2 dangerouslySetInnerHTML={{ __html: ctaheading }}></h2>
                    <Link
                        to={ctabuttonlink}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="default-btn-bordered"
                        dangerouslySetInnerHTML={{ __html: ctabuttontext }}
                    ></Link>
                </div>
            </div>
        </div>
